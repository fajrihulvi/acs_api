...
class Transaksi_Perubahan_Daya_Prabayar extends REST_Controller {
	...
	function index_get() {
		...
		if ($id == '') {
			$transaksi_perubahan_daya_prabayar = $this->db->get('m_transaksi_perubahan_daya_prabayar')->result();
		} else {
			$this->db->where('id', $id);
			$transaksi_perubahan_daya_prabayar = $this->db->get('m_transaksi_perubahan_daya_prabayar')->result();
		}
		...
		if($status){
			if ($transaksi_perubahan_daya_prabayar)
			{
				$this->response($transaksi_perubahan_daya_prabayar, REST_Controller::HTTP_OK); 
			}
		}
		else
		{
	...
	function index_post() {
		...
		$insert = $this->db->insert('m_transaksi_perubahan_daya_prabayar', $data);
		if ($insert) {
