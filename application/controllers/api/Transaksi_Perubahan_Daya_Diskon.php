...
class Transaksi_Perubahan_Daya_Diskon extends REST_Controller {
	...
	function index_get() {
		...
		if ($id == '') {
			$transaksi_perubahan_daya_diskon = $this->db->get('m_transaksi_perubahan_daya_diskon')->result();
		} else {
			$this->db->where('id', $id);
			$transaksi_perubahan_daya_diskon = $this->db->get('m_transaksi_perubahan_daya_diskon')->result();
		}
		...
		if($status){
			if ($transaksi_perubahan_daya_diskon)
			{
				$this->response($transaksi_perubahan_daya_diskon, REST_Controller::HTTP_OK); 
			}
		}
		else
		{
	...
	function index_post() {
		$data = array(
			...
			'diskon_bp'    => $this->post('diskon_bp'),
			'diskon_ujl'    => $this->post('diskon_ujl'),
			'nilai_maksimum'    => $this->post('nilai_maksimum'),
			'status'    => 'pending',
		);

		$insert = $this->db->insert('m_transaksi_perubahan_daya_diskon', $data);
		if ($insert) {
